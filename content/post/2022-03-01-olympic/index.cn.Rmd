...
```{r countNO, fig.cap="(ref:fig-countNO)", fig.show='hold'}
counts <- data %>% 
...
在最初的奥运会中，还存在艺术竞赛（Art Competition）项目的比拼，准确的说从`r range(art["Year"])[1]`年至`r range(art["Year"])[2]`年间的几届奥运会。
...
```{r art-competition, fig.cap="(ref:fig-art-competition)"}
...
```{r medal-arts, fig.cap="(ref:fig-medal-arts)"}
...
共有`r art_country`个国家参加了奥运会的艺术竞赛类项目，仅有不到一半数量的国家（`r art_country_medal`）获得了奖牌（如图\@ref(fig:medal-arts)所示），排名前三位的为德国、法国、意大利。
...
```{r counts-sex, fig.cap="(ref:fig-counts-sex)"}
...
```{r The-latest-Olympic-male-female}
counts_sex_latest <- counts_sex %>% 
  filter(Year == 2016)
counts_sex_latest
```

如图\@ref(fig:counts-sex)所示，直到1996年，参加奥运会的女运动员数量的变化趋势都与男运动员相同，此时男运动员的数量达到8000人的最顶点，而此时女运动员的数量还在以一个较高的增长率上升。最近的一届奥运会（2014年索契冬奥会和2016年里约奥运会中），女运动的数量已经超过了`r counts_sex_latest[2, 3]/sum(counts_sex_latest["Athletes"])`。
...
```{r 5-years-regression, fig.cap="(ref:fig-5-years)"}
...
medalChina <- data %>% 
  ...
  summarize(Count = length(Medal))
```
```{r china-medal, fig.cap="历届奥运会中国奖牌数量变化"}
...
```{r data-completeness, fig.cap="Height/Weight data completeness from each Olympics"}
...
```{r WH-overtime, fig.cap="Athlete height & weight over time"}
